...
export class TopicService {
  ...
  async createTopic(dto: CreateTopicRequest): Promise<CreateTopicResult> {
    ...
    const shouldAutoTitle = !requestedTitle;
    let title = requestedTitle;
    if (shouldAutoTitle) {
      // Don't block topic creation on a network call.
      title = await generateTopicTitleFromBody(body, { allowNetwork: false });
    }
    ...
    if (shouldAutoTitle) {
      // Fire-and-forget: try to upgrade the fallback title with an AI-generated one.
      this.upgradeAutoGeneratedTitle({ topicId, rootArgumentId, body, fallbackTitle: title }).catch((err) => {
        console.warn(`[topic] Failed to upgrade auto-generated title topicId=${topicId}:`, err);
      });
    }
  ...
  private async upgradeAutoGeneratedTitle(params: {
    topicId: string;
    rootArgumentId: string;
    body: string;
    fallbackTitle: string;
  }): Promise<void> {
    const generatedTitle = await generateTopicTitleFromBody(params.body);
    if (generatedTitle === params.fallbackTitle) return;

    const topic = await this.prisma.topic.findUnique({
      where: { id: params.topicId },
      select: { title: true, rootArgumentId: true },
    });
    if (!topic) return;
    if (topic.title !== params.fallbackTitle) return;
    if (topic.rootArgumentId !== params.rootArgumentId) return;

    const root = await this.prisma.argument.findUnique({
      where: { topicId_id: { topicId: params.topicId, id: params.rootArgumentId } },
      select: { title: true },
    });
    if (!root) return;
    if (root.title !== params.fallbackTitle) return;
    ...
    try {
      await this.topicEvents.publish(params.topicId, {
        event: 'topic_updated',
        data: { topicId: params.topicId, reason: 'auto_title_generated' },
      });
    } catch {
      // ignore
    }
    ...
    this.translations.requestTopicTitleTranslation({ topicId: params.topicId, title: generatedTitle }).catch((err) => {
      console.warn(`[topic] Failed to request title translation topicId=${params.topicId}:`, err);
    });
    this.translations
      .requestArgumentTranslation({ argumentId: params.rootArgumentId, title: generatedTitle, body: params.body })
      .catch((err) => {
        console.warn(`[topic] Failed to request root argument translation argumentId=${params.rootArgumentId}:`, err);
      });
  }
...
async function generateTopicTitleFromBody(
  body: string,
